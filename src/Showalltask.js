...
export function Showalltask({ obj, index, list, setList }) {
  ...
  function markascomplete() {
    let updatedarr = list.map((ele) =>
      ele.title === obj.title ? { ...ele, status: toup } : ele
    );
    setList([...updatedarr]);
  }

  function markaspending() {
    let updatedarr9 = list.map((ele) =>
      ele.title === obj.title ? { ...ele, status: toup } : ele
    );
    setList([...updatedarr9]);
  }
  ...
  function setreminder() {
    let updatedarr2 = list.map((ele) =>
      ele.title === obj.title ? { ...ele, reminder: toup2 } : ele
    );
    setList([...updatedarr2]);
    let temp1 = Number(
      prompt("Enter the delay after which you want the reminder in seconds")
    );
    reminderid = setTimeout(() => {
      let msg = obj.title + " : " + obj.content;
      alert(msg);
      let updatedarr3 = list.map((ele) =>
        ele.title === obj.title ? { ...ele, reminder: "no" } : ele
      );
      setList([...updatedarr3]);
    }, temp1 * 1000);
  }
  ...
  return (
    <div className="taskcont">
      ...
      <div className="crud">
        ...
        {obj.reminder === "no" && obj.status !== "complete" ? (
          <Button
